...
class MultiMiniGridEnv(gym.Env):
    ...
    def possible_actions(self):
        return [agent.possible_actions(self.grid) for agent in self.agents]
    
    def random_actions(self):
        return [agent.random_action(self.grid) for agent in self.agents]
    ...
    def step(
        ...
        for i, agent in enumerate(self.agents):
            obs, reward, terminated, truncated, _ = agent.step(action[i], grid=self.grid)
            total_terminated.append(terminated)
            total_truncated.append(truncated)
            total_reward += reward
        ...
        return np.array(total_obs).flatten(), total_reward, all(total_terminated), all(total_truncated), {}

    def get_pov_render(self, tile_size):
        """
        Render an agent's POV observation for visualization
        """
        _, vis_mask = self.gen_obs_grid(grid=self.grid)
...
class MultiMiniGrid10x10(MultiMiniGridEnv):
    ...
    def _gen_grid(self, width, height):
        ...
        for goal in goals:
            self.grid.set(goal[0], goal[1], Goal("green"))
